...
- [ ] Are tokens short-lived (typically 1-60 minutes), JWTs verified against a pinned algorithm allowlist and a server-side key (rejecting alg:none and any token-header-selected algorithm or key) before claims are read, and API keys cryptographically random (typically 32 characters / 256 bits)? — read auth middleware and key generation
- [ ] Are cryptographic primitives sound — no MD5/SHA-1 for signatures or token derivation, no ECB mode or broken/legacy ciphers (DES/3DES/RC4/Blowfish) for symmetric encryption, passwords via a salted adaptive KDF (bcrypt/scrypt/argon2/PBKDF2), and secrets/tokens/HMACs compared constant-time? — read hashing, password-storage, and secret-comparison sites, and grep cipher-construction sites for ECB selectors (AES.MODE_ECB, modes.ECB, CipherMode.ECB) and legacy cipher names appearing as the factory's algorithm argument (Cipher.getInstance, crypto.createCipheriv, openssl_encrypt, OpenSSL::Cipher.new, EVP_*, or pyca/cryptography algorithms.TripleDES/Blowfish/ARC4), matching the algorithm argument rather than bare identifier substrings, and excluding bcrypt's internal Blowfish, comments, and test fixtures
...
- [ ] Are cloud data-plane resources (object storage, databases, caches) free of public-internet grants in IaC — no public-read/public-read-write ACLs, allUsers/allAuthenticatedUsers bindings, allow_blob_public_access, or publicly_accessible? — grep IaC storage/database resource declarations for those flags
...
- [ ] Do XML parsers that receive user-controlled input have DTD processing and external-entity resolution disabled, or use a safe-by-default/defused parser? — grep XML parser instantiation (DocumentBuilderFactory, lxml/ElementTree, XmlReader, SimpleXML, libxmljs) for entity/DTD hardening
- [ ] Is a user-controlled path component reaching a file-serve, dynamic-include, or archive-extraction sink constrained to an intended base directory (canonicalize + base-prefix check, basename strip, or allowlist)? — grep send_file/sendFile/static-serve, dynamic include/require, and archive extractall sites for a dynamic path lacking a canonicalize-and-verify guard
