...
description: "Use when nontrivial repository work needs principal-level technical execution: architecture, system design, domain/API/data modeling, legacy change, refactoring, debugging, reliability, security, performance, runtime diagnosis, release risk, pre-landing readiness, and verification. Also covers CI/CD and DevSecOps pipeline design, secret management and security/scan governance, .NET/ASP.NET Core enterprise work, phased delivery planning, and authoring ADRs, RFCs, design docs, review reports, and executive summaries. Make sure to use when Copilot must implement changes, preserve behavior, reduce complexity, prevent staff-review findings, or explain engineering tradeoffs. Excludes PM/people/process/Agile/TDD/BDD/SDD advocacy, roadmap, and culture advice."
---
