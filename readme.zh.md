...
> 讓 AI coding agent 以 principal 等級的技術判斷執行工作——行動優先、以證據收斂、並用做 production 的紀律維護。

English: [readme.md](readme.md)

多數「資深工程師」提示詞讓 agent「聽起來」資深；這個 skill 是要讓它「表現得」資深：先讀懂系統再決策、動手改 repository 而不是說教、用實際命令、final diff 與明確停止條件證明變更——不然就講清楚為什麼做不到。精美報告不被當成可運作軟體的替代品。

它以單一對齊來源同時交付三個 host（Claude Code、Codex、GitHub Copilot），內含一層依領域分組、可對任何 repository 逐條套用的掃描判準，而且用它要求 agent 的同一套紀律來維護自己——重大變更前先做對抗式審查、以有紀錄的盲評 A/B eval 驗證能力、fail-then-pass 行為驗證，以及 CI 強制的套件對齊。
...
你不需要知道該提什麼需求。把一個 repo 交給它，它會自己盤點最高價值的工作，再端出一份菜單讓你選。它提供的服務，每一項都以實際掃描得到的證據為據，而非泛泛建議：

- **正確性與修 bug** — 潛在崩潰、邊界失敗、錯誤輸出
- **資安強化** — 依賴/供應鏈漏洞、注入、授權、機密、OWASP 面向
- **架構** — 模組邊界、耦合與內聚、god-file 拆解、依賴方向
- **Clean code / 重構** — 死碼、重複、命名、簡化、保行為的結構調整
- **測試覆蓋與安全網** — characterization 測試、覆蓋缺口、flaky 測試修復
- **效能** — 熱路徑、N+1、記憶體配置、bundle 大小、延遲
- **開發者體驗** — lint/format 工具、CI 回饋速度、腳本、上手文件、build 體感
- **CI/CD 與發布安全** — pipeline 缺口、required checks、branch protection、部署 gate
- **執行期診斷** — 事故、可靠性、容量、佇列
- **技術決策與遷移** — 框架/資料/服務選型、相容性、升級

當你只丟一句「維護」或「改善」某專案、沒有指定任務時，它會先掃健康訊號，然後端出一份精簡的 A/B/C/D 菜單，只列這個專案真正需要的軸——非專業使用者不必自己說出服務名稱。若請求模糊或打錯目標，它會提出一個更銳利、可驗證的目標讓你確認，而不是瞎猜。每次報告都以它找到、排序過的 next opportunities 收尾，各標「現在就能安全做」或「需要你決策」。

## 它用什麼態度看你的程式碼

它預設懷疑、標準很高——包括對自己。未經證明的結論（根因、「修好了」、「這樣安全」）會被當成可能的幻覺，除非證據逼它成立；回報前它會先試著推翻自己的說法。當你把一個專案交給它接手或維護，它會先用最嚴厲的一刀掃過——找 latent bug、隱藏假設、不安全的邊界、缺少的測試，才給任何肯定。它的吹毛求疵一定有 file:line 或可複現的案例撐著，而非憑品味；每個批評都指向具體的修法。刁鑽但有建設性：目標是更好的 repository，不是挑錯本身。
...
其中一份 reference 不是技術工具箱，而是一層可勾選的掃描判準：`audit-scan-checklists.md` 收錄唯讀、可對 repository 逐條檢查的判準，依領域分組（架構與耦合、API 與契約、領域與資料、分散式一致性、程式建構、錯誤處理、測試、安全、效能、維運、變更安全），另含給新 repository 的 Day-One Baseline。專案稽核會把個別判準當探針選用；新專案建置會拿 Day-One Baseline 對照自己的設定；使用者明確要求全面掃描時，才逐節掃過相關判準。每個命中都只是 candidate，要經過同一套 trigger/mechanism/impact/owner/evidence 紀律晉升才算 finding——這一層只提供可檢查的條件，不是把清單原樣傾倒出來。
...
- 用唯讀、可勾選的判準做全 repository 稽核：新專案可直接套用，現有專案可逐條 checkbox 掃描找出不足
- 依照變更影響範圍選擇合適的驗證方式
...
變更被當成工程來做，不是編輯。每個非瑣碎變更都要經過鏡像同步紀律、落地前的抗辯審查、行為層 fail-then-pass 驗證與內容忠實度規則；能力宣稱以盲評 A/B eval 檢驗，其 transcript 與分數留有紀錄，而非空口聲稱。方法論與其證據放在 [MAINTENANCE.md](MAINTENANCE.md) 與 [`evals/`](evals/)——改動 skill 內容前請先讀。
